...
# Event-Typen die in der Seitenleiste fett/prominent erscheinen
_PROMINENT_TYPES = {
    "av_started", "av_stopped", "input_started", "input_stopped",
...
def _event_label(ev: dict) -> str:
    ...
    if t == "mouse_click":
        ...
        parts.append(f"{ev.get('button','').replace('Button.','')}{action} ({ev.get('x','?')},{ev.get('y','?')})")
...
class _EventRow(QWidget):
    """Klickbare Zeile im Live-Event-Panel."""

    jumped = Signal(float)   # emittiert t_ms beim Klick
    ...
    def __init__(self, ev: dict, parent: QWidget | None = None) -> None:
        ...
        # Farbiger Dot
        color = _DOT_COLOR.get(ev.get("type", ""), _DEFAULT_DOT)
        dot = QLabel()
        ...
        # Detail (Koordinaten, Taste etc.) — nur für prominent
        if self._prominent:
            t = ev.get("type", "")
            detail = ""
            if t == "mouse_click":
...
class _BoundaryDialog(QDialog):
    """Zeigt gesetzte Grenzen und erlaubt das Löschen einzelner Einträge."""
    ...
    def __init__(self, boundaries: list[dict], parent: QWidget | None = None) -> None:
        ...
        self._boundaries = list(boundaries)   # lokale Kopie
...
class AnnotationWindow(QWidget):
    ...
    def load_session(self, session: RecordingSession) -> None:
        ...
        # Verzeichnisname im editierbaren Feld anzeigen
    ...
    def _build_event_panel(self) -> QWidget:
        ...
        self._event_scroll.setHorizontalScrollBarPolicy(
    ...
    def _build_bottom_bar(self) -> QWidget:
        ...
        # Transport + Grenze setzen
        transport_row = QHBoxLayout()
        transport_row.setSpacing(6)

        btn_start  = self._tbtn("⏮", "Zum Anfang")
        btn_back   = self._tbtn("◀ 5s", "5 s zurück")
        self._play_button = self._tbtn("▶", "Play / Pause")
        btn_fwd    = self._tbtn("5s ▶", "5 s vor")
        btn_end    = self._tbtn("⏭", "Zum Ende")
    ...
    def _tbtn(self, text: str, tooltip: str) -> QPushButton:
        b = QPushButton(text)
    ...
    def _on_media_status_changed(self, status) -> None:
        """Zeigt das erste Frame sobald das Video geladen ist."""
    ...
    def _on_slider_moved(self, value: int) -> None:
        if self._duration_ms > 0:
            ...
            # Frame anzeigen auch ohne laufendes Playback
            if self._player.playbackState() != QMediaPlayer.PlaybackState.PlayingState:
                self._player.play()
                self._player.pause()
    ...
    def _show_boundary_dialog(self) -> None:
        ...
        dlg.exec()  # immer speichern, egal ob Accept oder Escape/X
